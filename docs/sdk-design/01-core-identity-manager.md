...
| `logTrust` | Exactly one of `managed` ([managed catalog](11-c2sp-tlog-binding.md#identity-digital-managed-trust)), `profile` (inline [trust profile](11-c2sp-tlog-binding.md#trust-profiles) document), or `policyUrl` ([generic factory](11-c2sp-tlog-binding.md#verification-convenience-factory)). Zero or more than one fails with `ArgumentError`. |
