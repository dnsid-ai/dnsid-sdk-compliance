...
| COM-007 | Must P1 | Define security acceptance criteria for external release and material changes. | 🔧 | `ci/readiness.sh` is the acceptance gate: ❌ blocks; runs on every PR and weekly. **9/22: not running** — public SDK repos cannot call the private compliance repo's reusable workflow (M7); every run since publish is a startup failure. |
