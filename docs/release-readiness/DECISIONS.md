...
| M3 🟡 | **CLI release status.** *9/21: binary signing deferred to post-release; public-vs-internal still open.* The CLI lives in the private platform monorepo and ships as compiled binaries. Is it a public download at launch? If yes, binaries need signing, hashes, provenance, and their own export/installer review; if no, SDK docs that reference it need adjusting. | OSS-010/011/012, Legal §4 row 4, §5 row 6 | Internal-only at launch; SDK READMEs reference it as "the DNSid CLI" without a download link. | Publish (doc changes) |
