...
INFOSEC = {
 ...
 "COM-007": ("🔧", f"{R} is the acceptance gate: ❌ blocks; runs on every PR and weekly. **9/22: not running** — public SDK repos cannot call the private compliance repo's reusable workflow (M7); every run since publish is a startup failure."),
 ...
 "SSD-007": ("🔧", f"gitleaks full history in {R}; GitHub secret scanning + push protection enabled on all 4 public repos 9/22 (0 alerts)."),
 ...
 "VIR-001": ("🔧", f"SECURITY.md byte-checked against canonical ({R}). GitHub Security Advisories + email. Repos public + private vulnerability reporting enabled on all 4, 9/22."),
 ...
 "OSS-010": ("⏳", f"py: provenance ✅. ts: provenance ✅ from 0.20.2 (staged trusted publishing, `npm audit signatures` verified). go: deferred post-launch by decision (E5). {R} shows ❌ until done."),
 ...
 "OSS-023": ("📄", f"{TM} §C1–C7."),
 "OSS-024": ("⏳", f"Signing keys not in repos; py + ts provenance via OIDC; ts staged publish — CI alone cannot go live ({TM} C6). go → E5. Ruleset has no bypass."),
}
for i in range(1, 11):
...
LEGAL = [
 ("1. Approval to Publish, Licensing, and Distribution", [
  ...
  ("What is published vs held back is recorded", "📄", "`STATUS.md` repo table: 3 SDKs + cookbook public (9/22); witness pending W1–W8; compliance repo private (→ M7, now blocking: must go public or SDK CI gates stay broken) + platform monorepo (server, console, CLI) private → M3."),
