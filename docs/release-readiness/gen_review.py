...
INFOSEC = {
 ...
 "IAM-005": ("⏳", f"All org members on 2FA; org-level enforcement pending enterprise → {D} E4."),
 ...
 "VIR-003": ("⏳", f"Draft in `SECURITY-fixes-section.draft.md`; numbers → {D} L8."),
 "VIR-004": ("⏳", "Same draft §'How fixes are released' (new version + advisory; patch on latest minor)."),
 ...
 "VIR-008": ("⏳", "Draft: CVE via GitHub CNA for Medium+. Confirm with L8."),
 "VIR-009": ("⏳", f"SECURITY.md supported-versions table exists; support period → {D} L8."),
 ...
 "OSS-010": ("⏳", f"py: provenance ✅. go/ts: skipped for launch → {D} E5. {R} shows ❌ until done."),
 ...
 "OSS-019": ("⏳", "Dependabot for updates; deprecation/EOL → `SECURITY-fixes-section.draft.md` §EOL, numbers L8."),
 "OSS-020": ("🔧", f"SECURITY.md byte-checked ({R})."),
 "OSS-021": ("⏳", "Should/P2. Supported-versions table in SECURITY.md; lifecycle → L8."),
...
LEGAL = [
 ("1. Approval to Publish, Licensing, and Distribution", [
  ...
  ("Contribution intake decided (CLA / DCO / neither)", "⏳", f"{D} L5 — re-asked; default DCO."),
 ...
 ("9. Security and Vulnerability Handling", [
  ...
  ("Dependency/vuln scanning with an owner for findings", "⏳", f"Scanning ✅; owner → M4."),
 ...
 ("10. EU Cyber Resilience Act", [
  ...
  ("Reporting path for actively exploited vulns with named person", "⏳", f"**Obligation live since 11 Sep 2026.** → {D} E3."),
 ...
 ("12. Governance and Ongoing Operation", [
  ("Governance model recorded: who merges, who speaks", "⏳", f"Merging: CODEOWNERS + ruleset. Speaking: → {D} M8 (needs M4)."),
